...
{
    /// <summary>
    /// Логика взаимодействия для DecreesWindow.xaml
    /// </summary>
    public partial class DecreesWindow : Window
    {
        [Dependency]
        public new IUnityContainer Container { get; set; }
        GroupLogic _logic;
        public int _customerId { get; set; }
        public DecreesWindow(GroupLogic logic)
        {
            InitializeComponent();
            _logic = logic;
        }
        ...
        private void Delete_Click(object sender, RoutedEventArgs e)
        {
            if (DataGridView.SelectedIndex != -1)
            {
               ...
               MessageBoxImage.Question);
                ...
                {
                    GroupViewModel treatment = (GroupViewModel)DataGridView.SelectedCells[0].Item;
                    int id = Convert.ToInt32(treatment.Id);
                    try
                    {
                        _logic.Delete(new GroupBindingModel { Id = id });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButton.OK,
        ...
        private void Add_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var form = App.Container.Resolve<GroupWindow>();
